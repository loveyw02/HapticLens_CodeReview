...
def get_polar_grid(h, w):
    ...
    wx, wy = np.meshgrid(np.arange(-w2, w2 + (w % 2)) / w2, np.arange(-h2, h2 + (h % 2)) / h2)
...
def build_level(image_dft, filt):
    ...
    """
    return cp.fft.ifft2(cp.fft.ifftshift(image_dft * filt))
...
def recon_level(pyr_level, filt):
    ...
    """
    return 2.0 * cp.fft.fftshift(cp.fft.fft2(pyr_level)) * filt


def build_level_batch(image_dft, filt) -> cp.ndarray:
    """Builds a single level of the Pyramid Decomposition
    Inputs:
    ...
    Output:
        Pyramid decomposition as the level specified by filt
    """
    return cp.fft.ifft2(cp.fft.ifftshift(image_dft * filt, axes=(1, 2)), axes=(1, 2))


def recon_level_batch(pyr_level, filt) -> cp.ndarray:
    """Reconstructs a single Level of the Pyramid Decomposition
    Only Valid for Sub bands in Complex Pyramids (not lo or hi
    pass filter bands)
    Inputs:
    ...
    Outputs:
        recon_dft - Reconstructed DFT of the input Pyramid Level
    """
    return 2.0 * cp.fft.fftshift(cp.fft.fft2(pyr_level, axes=(1, 2)), axes=(1, 2)) * filt
