...
class SteerablePyramid:
    def __init__(self, depth, orientations, filters_per_octave=1, twidth=1.0, complex_pyr=False):
        """
        ...
        self.pyr_scale_factor = 2 ** (1.0 / filters_per_octave) # lambda from Phase-Based Frame Interpolation for Video Eq. 10
    ...
    def get_filters(self, h, w, cropped=False):
        """Builds Filters for a given set of reference image dimensions
        Inputs:
            h - reference image height
            w - reference image width
        ...
        lo_mask = lo_mask_prev # just initializing for pylance
        ...
        for idx, rval in enumerate(radial_vals[1:]):
            ...
            for b in range(self.orientations):
                ...
                direction = [-np.sin(np.pi * b / self.orientations),
                             np.cos(np.pi * b / self.orientations)]
                motion_dirs.append([direction[0] * scale_factor, direction[1] * scale_factor])
        ...
        motion_dirs.append([0, 0])

        return filters, crops, motion_dirs
    ...
    def display(self, filters, title=""):
        ...
        for i in range(self.num_filts):
            ...
            for j in range(1, self.orientations + 1):
                jdx = idx + j
                ax[i][j - 1].imshow(filters[jdx]) # type: ignore
...
class SuboctaveSP(SteerablePyramid):
    def __init__(self, depth, orientations, filters_per_octave, cos_order=6, complex_pyr=True):
        ...
        NOTE: there is an issue with non-complex pyramids, not yet sure what it is
        """
        self.depth = depth
        self.num_filts = depth * filters_per_octave
        self.orientations = orientations
    ...
    def get_filters(self, h, w, cropped=False): # type: ignore todo
        """Builds Filters for a given set of reference image dimensions
        Inputs:
            h - reference image height
            w - reference image width
        ...
        rad_filters = []
        total = np.zeros((h, w))
        const = np.power(2, 2 * self.cos_order) * np.power(factorial(self.cos_order), 2) / ((self.cos_order + 1) * factorial(2 * self.cos_order))
